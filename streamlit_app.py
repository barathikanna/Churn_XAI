...
@st.cache_data
def load_data():
    df = pd.read_csv("dashboard/causal_discount_churn_DAG_clean.csv")
    cf_df = pd.read_csv("dashboard/DiCE_counterfactuals_all.csv")
    X_train_raw = pd.read_csv("dashboard/X_train_raw.csv")

    # Ensure review_text exists and is string
...
def load_model_and_processed():
    with open("data/xgb_pipeline.pkl", "rb") as f:
...
# ---------------------------
    ...
    discount_suggestion = cust_cf_best.iloc[0]["discount_suggestion"] if not cust_cf_best.empty and "discount_suggestion" in cust_cf_best.columns else "N/A"
    ...
    # Render LIME HTML in-memory
    html_str = lime_exp.as_html()
